...
st_ok() {
    local ck="$C_B8/tools/checkasm/checkasm"
    # Every value goes through c_shq: this file is SOURCED, so each value
    # is code, and one apostrophe in a CPU model or an emulator note
    # silently swallows the lines beneath it.
    okv() { printf '%s=%s\n' "$1" "$(c_shq "$2")" >> "$C_OK"; }
    {
    ...
    okv BOOT_DATE "$(date -u +%Y-%m-%dT%H:%M:%SZ)"
    okv BOOT_CLOUD "$CLOUD"
    okv BOOT_EMULATOR "${C_EMULATOR:-}"
    okv Y264_CPU_NAME "$(c_y264_cpu_name "$ck")"
    okv CPU_MODEL "$(c_cpu_model)"
    okv CPU_FLAGS "$(c_cpu_flags)"
    okv NPROC "$(c_nproc)"
    okv KERNEL "$(uname -srm)"
    okv CC_VERSION "$(${CC:-gcc} --version 2>/dev/null | head -1)"
    okv CLANG_VERSION "$(clang --version 2>/dev/null | head -1)"
    okv MESON_VERSION "$(meson --version 2>/dev/null)"
    okv FFMPEG_VERSION "$(ffmpeg -version 2>/dev/null | head -1)"
    okv Y264_SRC "$C_SRC"
    okv Y264_COMMIT "$(git -C "$C_SRC" rev-parse HEAD 2>/dev/null)"
    okv Y264_BUILD8 "$C_B8"
    okv Y264_BUILD10 "$C_B10"
    okv X264_DIR "$C_X264"
    okv X264_ASM "$C_X264/x264-asm"
    okv X264_C "$C_X264/x264-noasm-autovec"
    okv X264_COMMIT "$(git -C "$C_X264" rev-parse HEAD 2>/dev/null)"
    okv FFBOARD_FFMPEG "$C_FFB/FFmpeg/ffmpeg"
    okv Y264LIB "$C_FFB/prefix-y264"
    okv X264LIB_ASM "$C_FFB/prefix-x264-asm"
    okv X264LIB_C "$C_FFB/prefix-x264-c"
    okv VMAF_BIN "$( [ -f "$CLOUD_WORK/vmaf.path" ] && cat "$CLOUD_WORK/vmaf.path" )"
    okv SDE64 "$( [ -f "$CLOUD_WORK/sde64.path" ] && cat "$CLOUD_WORK/sde64.path" )"
    okv CLOUD_WORK "$CLOUD_WORK"
    cat "$C_OK"
    return 0
}
