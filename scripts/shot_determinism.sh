...
#
# The promise is guaranteed at one thread (owner, 2026-09-25): with T=1 (the
# full encode and every alone encode at --threads 1) a GOP that does not
# reproduce is a FAIL. With threads (the default, T=4 and K frame threads) it
# is informational, since threaded output may vary: a WARN, exit 0 unless an
# encode fails. STRICT=1 makes the threaded WARN exit 1 as well.
#
# Usage: scripts/shot_determinism.sh [input.y4m] [extra yah264 args...]
#   default input: local/corpus/ms_cif_30.y4m; T=<threads> (default 4; T=1 is
#   the guaranteed form and forces K=1), K=<frame threads> (default 2),
#   CRF=<crf> (default 26), YAH264=<binary>.
set -euo pipefail
ROOT="$(cd "$(dirname "$0")/.." && pwd)"
IN="${1:-$ROOT/local/corpus/ms_cif_30.y4m}"; shift || true
BIN="${YAH264:-$ROOT/build/cli/yah264}"; T="${T:-4}"; K="${K:-2}"; CRF="${CRF:-26}"
[ "$T" = 1 ] && K=1
...
"$BIN" --input-y4m "$IN" --crf "$CRF" --threads "$T" --gop-threads "$K" --cut-split \
...
while read -r g a b; do
    ...
    if cmp -s "$W/seg$g.264" "$W/alone$g.264"; then echo "  ok   GOP $g frames $a-$((b-1)) ($(stat -f %z "$W/seg$g.264") bytes)"; elif [ "$T" = 1 ]; then echo "  FAIL GOP $g frames $a-$((b-1)): alone differs from its segment at --threads 1"; fail=2; else echo "  WARN GOP $g frames $a-$((b-1)): alone differs from its segment (informational)"; fail=1; fi
    n=$((n+1))
done < "$W/ranges.txt"
if [ "$fail" = 0 ]; then
    echo "shot_determinism: $n/$n GOPs reproduce alone (T=$T, K=$K, CRF $CRF, $(basename "$IN"))"
elif [ "$fail" = 2 ]; then
    echo "shot_determinism: FAIL a GOP does not reproduce alone at --threads 1 (guaranteed there)"
    exit 1
else
