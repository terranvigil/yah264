...
| `Y264_MBT_MISSWHY=1\|2` | **why a Phase A source had no pair legs to reuse**: splits MBT_SPLIT's `nobleg` bucket into anchor / untyped / null-leg / no-have, and `=2` prints one line per miss carrying the source's POC, its ring offset, the settled bound, the STORED pair's anchor POCs and the pair the walk WANTED. Needs `Y264_MBT_SPLIT=1` beside it, since the bucket it splits is that probe's. `=2` is t1 only: the lines interleave on the pool | closed the stage-3 memo question in one read. All 72 nobleg sources on sunflower_1080p and all 48 on bbb10s_1080p_o120 are window-frontier leaves with **no future anchor bracket at all** (`want <poc>/INT_MIN`), whose pair legs the next anchor's finalize has not written yet. Not a keying defect: the key is two anchor POCs and carries no thread id and no frame index. Counting only, default inert, verified md5-identical on/off |
...
| `scripts/instr-ratio.sh N` | instruction-count ratio against x264 beside the wall ratio | quotes the table's factor without timer noise. Two counters, one column pair: `/usr/bin/time -l` on macOS, `perf stat -x, -e instructions,cycles,task-clock` on Linux. **The Linux arm needs perf to be readable.** A stock kernel ships `kernel.perf_event_paranoid` at 2 or 4; the counter then comes back empty and the row prints `NO-COUNTER` rather than a plausible zero. `sysctl kernel.perf_event_paranoid=1`, or a privileged container, is the fix. Do not read it under an emulator: QEMU's and Rosetta's instruction counts are the EMULATOR's work, not the encoder's, so the x86 column is a rented-metal reading |
| `Y264_SIMD_FORCE=none\|sse4\|avx2\|avx512`, beside `Y264_ASM_OFF=<class>` | **the two SIMD ablation axes, and they answer different questions.** `Y264_ASM_OFF` takes one dispatch CLASS to its C fallback and leaves the rest as shipped, so its wall delta is what that class's kernels currently buy. `Y264_SIMD_FORCE` takes the whole binary down to one ISA TIER by clearing the feature bits above it, so one binary can be asked what the SSE4.2 build would have done on an AVX2 box: the tier A/B without a second build directory, with `YAH264_NO_ASM=1` as the control column. Neither moves a byte of output -- every kernel is bit-exact with its C reference -- which is why both are identity-cmp axes as well. `none` is the twin of `YAH264_NO_ASM`; the three x86 tier names are inert on aarch64, where there is one tier and nothing above it to cap. The BUILD-time twin is meson's `-Dsimd=`, which leaves the kernels out of the binary instead of routing past them |
...
| `Y264_DIAG_NOHPEL=1` | force `y264_me_mc_luma_s`'s non-hpel fallback everywhere. Tests whether a result depends on WHICH of the two motion-compensation paths a block took, since the fast path is gated on a per-thread hpel registry. It now covers every luma MC the analysis runs: the sub-pel probes, the RD candidates, and the P_Skip prediction. Both paths are byte-identical, so it is an identity-cmp axis as well |
...
| `Y264_MEREUSE_TRACE` | census of discarded-MV class (a): ref (r-1)'s winning 16x16 list-0 MV, scaled by temporal distance, as a seed for ref r's search. Read 2026-09-16 at medium, 60 frames, one thread: the scaled seed beats the median predictor the search starts from on 57-61% of ref>=1 searches (foreman_cif / park_joy_720p / crowd_run_1080p, mean full-pel SAD gain ~2200-2600 when it does) and beats or ties the full search's own winner on 11-15% (mean gain 220-520). Not a wash: a speed lead (better start point) and a small quality lead (search misses). Output-neutral, default inert, t1-only. Shipping the seed is an open item |
...
| `BANDS=deep ARM='<env>' python3 scripts/band_at_rate.py` | **the DEEP band (VMAF-NEG 55-83) at matched achieved bitrate**: the regime the standing band cannot see, and where the regime-shaped arms live. Ladders are solved onto x264's own deep curve by `scripts/calibrate_deep.py --write` (`ladders_deep.json`; 10 of 12 clips span it, ducks and park_joy honestly dropped). `BANDS=all` concatenates both ladders; the default `band` keeps every historical number comparable. Rungs print a model-derived `qp~` so regime knobs read in QP space. **The deep band's noise floor is MEASURED at ~+/-1.2 per clip** (a near-inert AC-gain 1.734 perturbation reads foreman -1.20, bus -0.91, akiyo +0.60), so a deep read inside +/-1.2 is the band talking, not the arm |
| `ARMS='a=<env>;b=<env>' python3 scripts/hd_band.py` | **the HD band: BD-VMAF-NEG on twelve 720p and 1080p clips, several arms in ONE run.** The standing ladder is seven CIF clips against five HD ones; the clips engineers test with are the HD ones at the rates they ship at, and docs/hd-parity-plan.md makes that band the bar every stage-2 candidate is read against. Five rungs, CRF 23 to 35, the DEFAULT side as the anchor, one anchor curve serving every arm in the run. **Anchor and arm run at the SAME five CRF values, and the BD is integrated over the range the two curves share**, which is what BD-rate is defined to do. It does NOT solve the arm onto the anchor's bytes any more. That solve could not be read below `medium`: the frame QP is an integer, so a clip's CRF-to-bytes curve is a staircase, a rung on a step edge has no CRF on either curve that produces the target bytes, and 89 of 180 solves at `fast` missed by over 2% with the miss then read as quality (`local/records/band-interp-2026-09-20.md`). **The guard is the rate overlap.** Every cell prints its rate overlap, its quality overlap and the rungs that survived, and a clip under `MIN_RUNGS` (4 of 5) is NOT SCORED. 4 of 5 is the ordinary reading, because two curves that do not coincide lose one endpoint at each end; 3 is a whole rung of dislocation. **A clip the guard refuses is re-read on rungs moved to the middle of their steps** -- fix A, and those rows print `[A]`. The anchor's ladder is probed once per clip and preset, two encodes a rung, and cached in `$Y264_CACHE/hdband/ladder.json` keyed by the binary's sha as ffboard's solve cache is. A moved rung narrows the staircase offset without removing it, so the record says which clips were read that way. **Both near-null controls run in every band by default** (`Y264_MBT_DEPFLOOR=16` and `=8`; `CONTROLS=none` drops them). Each arm prints raw and control-referenced, and where the two controls disagree on a clip by more than the bar the band prints `UNRESOLVED` rather than `PASS`, because a bar under the instrument's own spread is not a reading. Every (clip, arm, rung) cell is cached in `$Y264_CACHE/hdband/points.json` under the same key shape, and the hit and miss counts print per clip and per run, so a band that reused a table cannot be mistaken for one that measured it; `NO_POINT_CACHE=1` bypasses it. `SOLVE=1` restores the matched-byte ladder and its `UNBRACKETED` class, unchanged, for the pages that document it. `CLIPS`, `RUNGS`, `FRAMES`, `JOBS`, `MIN_RUNGS`, `FIXA`, `PROBE_D`, `BAR`, `MEDIAN_BAR`, `CONTROLS` override; `WINDOW` and `ITERS` belong to `SOLVE=1`. Cost is 45 encodes and 45 VMAF scores a clip at eight arms, against 245 encodes under the solve. Load-immune, so it runs beside other work inside the core cap |
...
| `scripts/shot_determinism.sh [input.y4m]` | **the per-shot re-encode contract**: encodes with GOP boundaries on the cuts and every GOP to its own segment, then re-encodes each GOP's frames alone with the same pinned frame threads and compares bytes. The promise an orchestrator's convex-hull probing and partial re-encode stood on (docs/engine-interface.md); since 2026-09-25 it reports a mismatch as WARN and exits 0 (`STRICT=1` to fail); K and CRF from the environment |
...
| `scripts/calibrate_band.py --write`, `ladders.json`, `curves.json` | the band ladders themselves and the x264 curve they were solved onto. **A bare run writes nothing into the tree.** Without `--write` the two json files land in a fresh scratch directory, and the run prints its path. Every run prints both destinations, each marked `OVERWRITE` or `create`, before the first encode starts. `--dry-run` prints that plan and stops, which is how you check a command without spending the hours. `--clips`, `--crfs`, `--frames` and `--jobs` cut a full sweep down to a cell or two, for the same reason. `--out-dir` names a destination of your own |
| `scripts/calibrate_deep.py --write`, `ladders_deep.json`, `curves_deep.json` | the same, for the deep band (VMAF-NEG 55-83) over CRF 30-50. Same flags, same rule: the tree's ladders move only under `--write` |
...
| `scripts/ffboard.py` | **the control for "is the table scoring our Y4M reader?"** Both encoders called as libraries inside ONE ffmpeg process: one demuxer, one thread pool, no CLI on either side. It answers the input-path question rather than replacing the table -- run against `parity-status-crf` at the same points the two agree within noise on every clip, so the input path is not where the gap lives. Needs an ffmpeg built with libyah264 at `../build/ffboard/ffmpeg-yah264/ffmpeg` (docs/ffmpeg-integration-plan.md rebuilds it; it lived under `/tmp` until macOS swept that away on 2026-09-17). **Point `X264LIB` at a libx264 whose `-fno-tree-vectorize` was stripped** or the pure-C arm reads our vectorized C against their scalar C, worth a third of goal 2 |
| `scripts/ffboard.py` solve cache | **keyed by the three binaries, not by the clip alone.** `$Y264_CACHE/ffboard/solve.json` (default `~/.cache/yah264/ffboard`) holds the CRF and ABR solves and survives between boards and between weeks. **It moved out of `$TMPDIR` on 2026-09-19**, where a sweep or a disk-full clear-out would have taken a record that is expensive to rebuild; the board's per-run bitstreams and y4m stay in scratch and are removed when it exits. Until 2026-09-18 the key was clip, frames, target, preset, threads and tolerance, with nothing in it about which libraries produced the answer. A cache from the libnext264 era was reused against a yah264 whose CRF scale had moved since, and the board came out at dsize -12% on the median, -24 to -37% on five clips, with every wall ratio on it meaningless. The key now carries a short sha of each dylib the ffmpeg will actually load (leaf name from `otool -L`, symlinks resolved, because a prefix is rebuilt in place and a path is not an identity), each library's pkg-config `Version`, and a sha of the ffmpeg binary. **Every solve prints `solve cache: hit`/`miss` with that key**, because a board that reused a whole table used to look exactly like a board that solved one. `--no-solve-cache`, or `Y264_NO_SOLVE_CACHE=1`, neither reads nor writes it |
| `scripts/ffboard.py` size-match guard | **the board refuses to print a median it cannot stand behind.** Every `RC=crf` and `RC=abrm` row is rate-matched by construction, so an unmatched dsize means the two encoders were timed doing different amounts of work and the ratio is partly the bit difference. After the table, a dsize median outside +/-2% or any one clip outside +/-5% prints `INVALID: sizes unmatched`, names the worst clip, and exits 2 without adjudicating the PSNR leg. Two bars, because they catch different failures: the median catches a scale that has moved under the whole table, the per-clip bar catches the single solve that did not converge. `RC=abr` is exempt and says so on the line -- it hands both encoders the same target and does not deliver the same bits, which is what its `n rate` / `x rate` columns are for. `Y264_DSIZE_TOL` and `Y264_DSIZE_TOL_MAX` move the bars, and setting the first turns the guard on at `RC=abr` as well |
| `scripts/ffboard.py` header check | **the ffmpeg wrapper is ABI-bound to the `include/yah264.h` it was compiled against, and the board is where that gets noticed.** The build writes `git hash-object include/yah264.h` to `yah264-header.sha` beside the binary; the board hashes `$Y264LIB/include/yah264.h` and refuses a mismatch, naming both blobs and both paths. It exists because on 2026-09-19 a wrapper one field behind the library read `yah264_param_t` at the wrong offsets, opened, encoded, and solved park_joy to CRF 42.7 at 55 Mbps. The library's own guard is the other half (ABI 3: the param struct carries its own `size` and `yah264_encoder_open` refuses a foreign one), and this one needs no cooperation from the library at all. `Y264_HEADER_CHECK=0` bypasses it, for a run that is reproducing the failure on purpose. **`Y264LIB` is less authoritative than it looks**: `DYLD_LIBRARY_PATH` only supplies the leaf name the binary asks dyld for, so an install whose soname differs is not loaded at all and the build-time absolute path is used instead, silently |
...
| `KEEP_SCRATCH=1` | **keep a harness's working directory instead of removing it, and print where it is.** Every harness here builds its scratch through `scripts/scratch.sh` or `scripts/scratch.py`, which remove it on exit, on Ctrl-C and on a TERM from a killed agent. Set this when a cell failed and the bitstream or the recon is the evidence. It is the ONLY way any of these directories survives its run, on purpose |
| `make clean-scratch` | **the sweep for scratch no trap could remove** -- a SIGKILL, a panic, a power cut. It matches only the prefixes in `scripts/scratch-prefixes.txt`, only directories, only under `$TMPDIR`, and only ones older than a day, and prints each path with its size. `DRY=1` lists without removing; `DAYS=7` widens the age. Caches that are MEANT to outlive a run are not under `$TMPDIR` at all: `ffboard.py`'s `solve.json`, `bdcompare.py --cache` and the openh264 raw planes live in `$Y264_CACHE` (default `~/.cache/yah264`), which nothing sweeps |

## 5. Is it correct? (the gates every ship passes)

```
make test                                   # unit, 9/9 (checkasm is one of them)
./build/tools/checkasm/checkasm             # every kernel against its C reference, guards on
./build/tools/checkasm/checkasm --list      # the table: group, family, the ISA each row needs
./build/tools/checkasm/checkasm --isa neon pixel    # one tier, one family
...
scripts/determ_repeat.sh                    # SAME config, N times -> how many bitstreams (diagnostic; STRICT=1 fails)
...
scripts/x86-docker.sh                       # the whole battery as linux/amd64, under Rosetta
scripts/x86-docker.sh --backend qemu        # ...and again under QEMU, an item's final gate
...
| instrument | answers | notes |
|---|---|---|
| `tools/checkasm` | does every optimized kernel still produce exactly what its C reference produces, and does it stay inside the memory it declares | **49 groups, and what changed on 2026-09-17 is what a green run MEANS.** Each group compares the C reference against the kernel SYMBOL. It used to compare the two through the public entry point with `Y264_ASM_OFF` flipped in between -- a harness an environment variable could turn into a C-against-C tautology -- and that shape also left every kernel the dispatcher does not select untested: on a FEAT_DotProd box the plain-NEON 16x16 SAD and variance kernels, which still ship and still run on a box without dotprod, had never once been exercised, and `dequant_4x4/8x8` and `ssd_16xh/8xh` had no group at all. All four, and both dotprod twins, are rows now. Three nets sit on top of the value comparison: a poisoned border on every destination, a row-padding check for a kernel that stores a wide vector into a narrow block, and an **mmap/mprotect page guard** sized to exactly the window the kernel declares and run at BOTH tails -- the only net that catches a read whose value is discarded (the classic "load 8 lanes, use 4" tail: the scramble check cannot see it because the output does not move, and the sanitiser is quiet because the bytes are still inside the allocation). A page fault is a hard stop naming the shape rather than a counted failure. `--list` prints the table with the ISA each row needs; `--isa neon\|sse4\|avx2\|avx512\|all` runs one tier. The table is per ARCHITECTURE, so the group count is too: an aarch64 build runs the 49 above, and an x86-64 build runs the portable rows plus the x86 ones -- 84 groups since wave 3b, 38 per tier (15 pixel, 6 mc, 11 transform, 3 predict, 3 deblock) and 46 of them at `--isa sse4`. Since that wave a group's BODY is written once and takes its kernel as an argument, with one thin row per tier on top, so a new tier's twin inherits the adversarial fills and the page guards of the row it joins instead of restating them; wave 3b put the last two families on that shape, and the move brought four page guards the NEON rows had never had -- the two luma deblock edge windows, the chroma edge's four rows, every prediction destination block (packed, so a store sized to the register faults rather than merely disagreeing) and the 32-byte flat intra8x8 edge array. `--bench` is informational and single-threaded: a checkasm multiple is necessary, never sufficient |
| `scripts/x86-docker.sh` ("x86 in Docker") | does this encoder build and pass its correctness battery as linux/amd64, on a box that has no x86 in it, and does the SIMD tier axis move a byte | **The matrix is `{rosetta, qemu}` x `{Y264_SIMD_FORCE=none, sse4, avx2}` x `{make test incl. checkasm, conformance --fast, identity cmp against `none`}`, plus a 10-bit build**, out of `docker/x86/Dockerfile` (ubuntu 24.04; no libvmaf-dev, because Ubuntu packages none and every quality harness here reaches VMAF through ffmpeg anyway). The tier axis is the RUNTIME cap, so one build serves all three rows; `-Dsimd=` is the build-time twin and is CI's job. **What an emulated pass is worth is the whole question, so the script says it out loud.** Neither emulator has AVX-512, and the script ASSERTS its absence and prints the assertion, because an emulated green that nobody qualified is how "AVX-512 works" gets into a document without ever having executed. The two backends fail differently and both are needed: Rosetta translates ahead of time, is near native, and has a history of illegal-instruction bugs, so a case that dies of SIGILL is rerun under QEMU and its Rosetta row is marked "emulator" rather than passed; QEMU's TCG interprets, is several times slower, and is the independent second opinion, reserved for an item's final gate. The feature probe differs by backend too: under Rosetta `/proc/cpuinfo` is an x86 one and is printed, under qemu-user it is the HOST's arm64 file and says nothing, so the probe that counts on both is the binary's own CPUID as `checkasm --list` prints it. Selecting QEMU means DISABLING the Rosetta binfmt entry for the leg, since Docker Desktop registers both interpreters for the same ELF magic; the script announces that, traps it and restores it. **The negative control is the reason to run this before the kernels land**: on a tree with no x86 kernels, `sse4` and `avx2` must be byte-identical to `none` by construction, so a difference is a defect in this harness and not a finding about the encoder. Corpus clips are bind-mounted READ-ONLY at their own paths, symlink chain and all, and never copied: the corpus is ~25 GB and is not ours to redistribute. It reads NO clocks -- an emulated wall time is not a measurement -- and an emulated run is CPU load like an encode, so it is coordinated the way a timed leg is (docs/x86-plan.md, "Box rule"). The first thing it found was not an x86 defect at all: a stray `#endif` had left ~170 lines of the CLI inside an `#if defined(__APPLE__)`, so the CLI had not compiled on Linux on ANY architecture, and nothing had asked the question because the ubuntu CI job was manual-only The first thing it found was not an x86 defect at all: a stray `#endif` had left ~170 lines of the CLI inside an `#if defined(__APPLE__)`, so the CLI had not compiled on Linux on ANY architecture, and nothing had asked the question because the ubuntu CI job was manual-only. The fixture cache is keyed by the **ffmpeg identity** (the binary's path and its version banner) beside `FIXVER`, because the synthetic clips are written by ffmpeg and two ffmpegs do not write the same Y4M header: shared, a container run changed the result of the next native run in the same tree. The container still gets its own cache directory mounted over the tree's, which is also what keeps its writes out of the worktree The first thing it found was not an x86 defect at all: a stray `#endif` had left ~170 lines of the CLI inside an `#if defined(__APPLE__)`, so the CLI had not compiled on Linux on ANY architecture, and nothing had asked the question because the ubuntu CI job was manual-only | | |
| `scripts/hrd_check.py` | would a conforming decoder's coded picture buffer survive this stream: underflow, overflow, peak fullness, tightest arrival margin | Takes NOTHING about the bucket on the command line: BitRate, CpbSize, cbr_flag, the initial delay and every removal time come from the SPS VUI hrd_parameters and the buffering_period / pic_timing SEI. Exit 0 clean, 1 violation, **3 = the stream carries no HRD and nothing was checked** -- which is every stream without `--nal-hrd`, and a vacuous pass there would read as a green gate on every VBV stream in the tree. `--allow-no-hrd` softens that for callers that want it. Since B-hrd, `--nal-hrd vbr|cbr` makes yah264's own streams readable by it, and `HRD=1 scripts/cvbr_compliance.sh` is the 36-cell leg: it prints the VBV verdict beside both HRD verdicts, because a cell whose rate control already underflows its own model emits a stream that underflows the declaration of that model, and a raw HRD count would score the rate control while calling it a signalling result. **Not a replacement for `vbv_check.py`**, which is TOLD the bucket and simulates the ENCODER's law: that one asks "did our rate control respect the cap we gave it", this one asks "would a receiver's buffer survive what we emitted" |
...
| `scripts/recovery_check.py` | does an `--open-gop` stream really recover where its recovery point SEI says it does | Cuts the elementary stream at the chosen recovery point, pastes the parameter sets in front, decodes that, and compares the LAST `n - k` frames against the encoder's own `--dump-recon`. The tail is the whole trick: the pictures the recovery point does not cover are the ones with a picture order count below its own, they land at the FRONT in output order, and counting back from the last frame makes their number irrelevant. **The cut decode passes `-flags2 +showall`, and the version floor for that is ffmpeg 6.1.** Which pictures a decoder hands to a display after a mid-stream start is a display policy and not a decode result, and the versions disagree about it on the same bytes: 9.0.1 emits the covered pictures and suppresses the uncovered leading B frames, 6.1.1 emits the first of them and then stops two short, never draining the last two out of its buffer when the bitstream ends. The second is a shortfall against the output process 8.2.1 and Annex C describe -- every picture still in the buffer is output at the end of the bitstream -- but it is a policy defect and NOT a decode defect: asked for all of them, 6.1.1 produces the same samples 9.0.1 does, to the byte, on all three cells. So the selection is made here rather than borrowed from the decoder, which is what makes the three `ogop_rec_*` cells read the same under either version. Exit 0 clean, 1 mismatch, **2 = the stream carries no recovery point**, which for an `--open-gop` encode is itself a failure and is why it is not 0 |
...
| `X264LIB`, `Y264LIB` | `scripts/ffboard.py` | installed prefixes of libx264 and libyah264 for an ffmpeg that links both. They live at `../build/ffboard/x264asm`, `../build/ffboard/x264noasm` and `../build/ffboard/y264inst`, and they have no defaults on purpose: a stale install that loads reads as a result, a missing one reads as an error. Since 2026-09-19 the board hashes both dylibs into the solve-cache key and checks `$Y264LIB/include/yah264.h` against the header the ffmpeg was built with, so a stale install is now named rather than measured |
| `FF` | `scripts/ffboard.py` and the row scripts | that ffmpeg (default `../build/ffboard/ffmpeg-yah264/ffmpeg`, then the old `/tmp/ffmpeg-yah264/ffmpeg`). **The set moved out of `/tmp` on 2026-09-17, because macOS swept it and the board lost its in-process arm.** `Y264_FFBOARD_ROOT` moves the whole set; docs/ffmpeg-integration-plan.md rebuilds it |
...
- **Nothing else re-runs one configuration against itself.** `stair_determ.sh`
  ...
  lottery ticket for that day. `scripts/determ_repeat.sh` is the instrument
  for it (informational since 2026-09-25; `STRICT=1` fails on a difference),
  verified by failing against the binary it was written for.
...
- **A full temp volume does not read as a full temp volume; it reads as a
  content failure.** Two weeks of harness runs left 409 GB under `$TMPDIR`
  (9772 directories, none of them removed by the run that made them) and the
  data volume hit 100% on 2026-09-19. What the merge gate showed was twenty
  conformance cells failing with **"no geometry in the recon y4m header"** on
  the big qp0 clips: the recon had been truncated to nothing, and the cells
  were read as an encoder defect first. `conformance.sh` and `ffboard.py`
  now refuse to start below 20 GB free and name `make clean-scratch`; the
  harnesses remove their own scratch on every exit path. If a gate starts
  failing on the largest clips only, check `df` before reading the failure.
