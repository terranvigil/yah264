...
| `ARMS='a=<env>;b=<env>' python3 scripts/hd_band.py` | **the HD band: BD-VMAF-NEG at matched achieved bitrate on twelve 720p and 1080p clips, several arms in ONE run.** The standing ladder is seven CIF clips against five HD ones; the clips engineers test with are the HD ones at the rates they ship at, and docs/hd-parity-plan.md makes that band the bar every stage-2 candidate is read against. Five rungs, CRF 23 to 35, with the DEFAULT side as the anchor: its bytes need no solving and one default curve serves every arm in the run, where `band_at_rate.py` re-solves it per arm. Each arm is solved inside a +/-1.5 CRF window around the anchor rather than bisected from 10 to 48, and **a solve that converges onto an edge prints `UNBRACKETED` and is dropped rather than scored** -- the saving must not be able to fake a result. `CLIPS`, `RUNGS`, `FRAMES`, `JOBS`, `WINDOW`, `ITERS` override. Load-immune, so it runs beside other work inside the core cap; budget about half an hour per 720p clip and an hour per 1080p one per four arms |
...
| instrument | answers | notes |
|---|---|---|
| `tools/checkasm` | does every optimized kernel still produce exactly what its C reference produces, and does it stay inside the memory it declares | **49 groups, and what changed on 2026-09-17 is what a green run MEANS.** Each group compares the C reference against the kernel SYMBOL. It used to compare the two through the public entry point with `Y264_ASM_OFF` flipped in between -- a harness an environment variable could turn into a C-against-C tautology -- and that shape also left every kernel the dispatcher does not select untested: on a FEAT_DotProd box the plain-NEON 16x16 SAD and variance kernels, which still ship and still run on a box without dotprod, had never once been exercised, and `dequant_4x4/8x8` and `ssd_16xh/8xh` had no group at all. All four, and both dotprod twins, are rows now. Three nets sit on top of the value comparison: a poisoned border on every destination, a row-padding check for a kernel that stores a wide vector into a narrow block, and an **mmap/mprotect page guard** sized to exactly the window the kernel declares and run at BOTH tails -- the only net that catches a read whose value is discarded (the classic "load 8 lanes, use 4" tail: the scramble check cannot see it because the output does not move, and the sanitiser is quiet because the bytes are still inside the allocation). A page fault is a hard stop naming the shape rather than a counted failure. `--list` prints the table with the ISA each row needs; `--isa neon\|sse4\|avx2\|avx512\|all` runs one tier. The table is per ARCHITECTURE, so the group count is too: an aarch64 build runs the 49 above, and an x86-64 build runs the portable rows plus the x86 ones -- 52 of those since the transform, quant and scan families joined the pixel family at both tiers (waves 1 and 3a), 15 pixel rows and 11 transform rows per tier. Since that wave a group's BODY is written once and takes its kernel as an argument, with one thin row per tier on top, so a new tier's twin inherits the adversarial fills and the page guards of the row it joins instead of restating them. `--bench` is informational and single-threaded: a checkasm multiple is necessary, never sufficient | `tools/checkasm` | does every optimized kernel still produce exactly what its C reference produces, and does it stay inside the memory it declares | **47 groups, and what changed on 2026-09-17 is what a green run MEANS.** Each group compares the C reference against the kernel SYMBOL. `--list` prints the table with the ISA each row needs; `--isa neon\|sse4\|avx2\|avx512\|all` runs one tier, and the x86 slots are live with no rows behind them, so the kernel items of docs/x86-plan.md only ever ADD rows. `--bench` is informational and single-threaded: a checkasm multiple is necessary, never sufficient | `tools/checkasm` | does every optimized kernel still produce exactly what its C reference produces, and does it stay inside the memory it declares | **49 groups, and what changed on 2026-09-17 is what a green run MEANS.** Each group compares the C reference against the kernel SYMBOL. |
