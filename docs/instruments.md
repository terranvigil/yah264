...
| `X264LIB`, `Y264LIB` | `scripts/ffboard.py` | installed prefixes of libx264 and libyah264 for an ffmpeg that links both. They live at `../build/ffboard/x264asm`, `../build/ffboard/x264noasm` and `../build/ffboard/y264inst`, and they have no defaults on purpose: a stale install that loads reads as a result, a missing one reads as an error |
