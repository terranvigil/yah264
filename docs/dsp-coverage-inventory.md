...
- `tools/checkasm/checkasm.c` covers 44 kernel groups. It does **not** test `y264_dequant_4x4_neon`, `y264_dequant_8x8_neon`, `y264_ssd_16xh_neon` or `y264_ssd_8xh_neon` (grep for `dequant`/`ssd` in the file returns nothing). Those four ship untested against their C references.
...
| (not in table) | scalar SSD loop in `ssd_block`, macroblock.c:879-885 | `y264_ssd_16xh_neon` :18, `y264_ssd_8xh_neon` :30 | **none** |

Pixel table: 26 slots, 21 with a NEON twin, 5 without (the 4-wide and 8x4 SADs, `sad_x4` 4x8/4x4). Plus SSD, dispatched from the encoder with a NEON twin but no checkasm group.
...
| `y264_dequant_4x4` :419 | scalar :934-961 | `y264_dequant_4x4_neon` :67 | QUANT and flat | **none** |
| `y264_dequant_8x8` :388 | scalar :676-699 | `y264_dequant_8x8_neon` :91 | QUANT and flat | **none** |
...
checkasm (`tools/checkasm/checkasm.c`, 1699 lines, one `main`) tests 44 named groups: sad x7, sad_x4 x7, satd4x4, satd8x8, satd16x16, sa8d8x8, sa8d16x16, satd_x4_8x8, hadamard_ac8x8, texture_ac4, texture_ac48, var16x16, intra4x4_x9, intra_satd_x3_16, fdct4x4, idct4x4, fdct8x8, idct8x8, quant_4x4, quant_4x4_f64, quant_8x8, quant_8x8_f64, sub4x4_dct, add4x4_idct, sub8x8_dct8, add8x8_idct8, sub_dct4_blocks, zigzag_abs_8x8, scan_mask_8x8, zigzag_scan_4x4, deblock_luma_v4, deblock_luma_h4, deblock_chroma8_h, deblock_strength, intra16x16, intra_chroma, intra4x4, intra8x8, intra8x8_edge, mc_chroma, pixel_avg_wt, pred_copy/avg2, mc_luma, hpel_build; plus `--bench` rows and three trellis/est_decision microbenches (:1512-1690). **Untested NEON: `dequant_4x4_neon`, `dequant_8x8_neon`, `ssd_16xh_neon`, `ssd_8xh_neon`.** (The quant groups compare the dispatched `!w` call against the dispatched `w=flat16` call; the latter takes the scalar path because NEON requires `!w`, so they do test NEON vs C.)
