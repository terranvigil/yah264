...
The full version tries each shot a few ways and keeps the best. A separate tool encodes every shot at several quality settings, in parallel, compares them, and stitches the winners into one file. That depends on one guarantee: a shot encoded on its own comes out byte-for-byte the same as it would inside a full encode of the film. Most encoders can't promise that, because rate control, lookahead and thread scheduling carry state across shots. yah264 can, because every shot starts fresh at its keyframe and the encoder is deterministic at a fixed thread count. So a trial encode of one shot is an exact preview of the final file, and a shot that needs a different setting is redone alone and dropped back in, with the rest untouched. To support this the encoder provides the shot table, a plan of keyframes and per-shot settings, per-frame stats, and each shot as its own file. Both modes are off unless you ask for them. [engine-interface.md](docs/engine-interface.md) has the details.
