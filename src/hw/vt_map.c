...
static const struct y264_hw_opt table[] = {
    ...
    { "--crf-max",          Y264_HW_IGNORED, "the hardware's buffer loop is its own" },
    { "--ratetol",          Y264_HW_IGNORED, "the hardware's rate loop is its own" },
    { "--qpfile",           Y264_HW_IGNORED, "no per-frame type or QP force on the hardware" },
